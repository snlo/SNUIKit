...
@interface SNTableViewRowAction : NSObject<NSCopying>
{
...
@implementation SNTableViewRowAction
...
+ (instancetype)rowActionWithStyle:(UITableViewRowActionStyle)style title:(NSString *)title handler:(void (^)(UITableViewRowAction *, NSIndexPath *))handler {
    SNTableViewRowAction *rowAction = [[SNTableViewRowAction alloc] init];
...
+ (instancetype)rowActionWithStyle:(UITableViewRowActionStyle)style image:(UIImage *)image handler:(void (^)(UITableViewRowAction * _Nullable, NSIndexPath * _Nullable))handler {
    SNTableViewRowAction *rowAction = [self rowActionWithStyle:style title:@"holder" handler:handler];
...
__attribute__((constructor)) static void SNTableViewRowActionPatchEntry(void) {
...
#error Unsupported CPU
            ...
            if (tableViewRowActionClassLocation && !*tableViewRowActionClassLocation) {
                Class class = objc_allocateClassPair(SNTableViewRowAction.class, "UITableViewRowAction", 0);
                if (class) {
